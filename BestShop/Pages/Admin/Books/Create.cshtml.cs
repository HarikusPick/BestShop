...
{
    [RequireAuth(RequiredRole ="admin")] //require auth attribute a admini göndererek sayfaya eriþim saðlýyoruz
    public class CreateModel : PageModel
    {
        ...
        private IWebHostEnvironment webHostEnvironment;  //kök dizine ve saat birimine ulaþmak için kullanýlýr .core
        ...
        public void OnPost()
        {
            if (!ModelState.IsValid) // ! gelen veriler doðrulama kriterlerini karþýlamamýþ demektir.
            {
            ...
            string newFileName = DateTime.Now.ToString("yyyyMMddHHmmssfff"); //dosya adýný unique yapýyoruz süre ile
            newFileName += Path.GetExtension(ImageFile.FileName);

            string imageFolder = webHostEnvironment.WebRootPath + "/images/books/"; //concatenation -> birleþtirme iþlemi
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(sql, connection))
                    {
                        ...
                        command.ExecuteNonQuery(); // insert update gibi iþlemlerde kullanýlýr
                    }
                }
            }
            catch (Exception ex)
