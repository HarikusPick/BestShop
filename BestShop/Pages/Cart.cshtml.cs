...
namespace BestShop.Pages
{
    ...
    public class CartModel : PageModel
    {
        ...
        public decimal shippingFee = 35;
        public decimal taxRate = 0.20m; //m decimal türünde ondalýk sayýlarda kullanýlýr
        ...
        public void OnGet()
        {
            ...
            //action can be null, "add" , "sub", "delete"
            ...
            if (action != null && id != null && bookDictionary.ContainsKey(id)) 
            {
                ...
                - to set the shopping cart size using updated cookie
                */
                //Response.Redirect(Request.Path.ToString()); //url deki query komutlarýný gizliyoruz
                return;
            }
            try
            {
                        ...
                        using (SqlCommand command = new SqlCommand(sql, connection))
                        {
                            ...
                            using (SqlDataReader reader = command.ExecuteReader())
                            {
                                if (reader.Read())
                                {
                                    ...
                                    item.bookInfo.CreatedAt = reader.GetDateTime(9).ToString("dd/MM/yyyy");
                                    ...
                                    decimal calculate = item.bookInfo.Price * taxRate; //vergi hesaplama
                                    subtotal += item.totalPrice;
                                    total = subtotal + shippingFee + calculate;
                                    string x = total.ToString("0.##");
                                    printX = x; //küsürat silme
                                }
                            }
                        }
                    }
